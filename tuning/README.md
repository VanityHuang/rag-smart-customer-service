...
🎯 质量差距 0.37 ≥ 0.3 → 推荐质量更高的 k=15
⚙️  最终推荐: k=15 | Token=3076/请求 | 延迟=6.7s | 质量=4.10/5
```

### 评判标准

- **成本红线**：超过最低 Token 消耗 50% 的候选淘汰
- 幸存者中质量差距 < 0.3 → 选 Token 更少
- 质量差距 ≥ 0.3 → 选质量更高，注明多花的 Token

### 操作

```python
retriever_k = 15   # ← 改为推荐值
```
